...
res_name = "LR search " + res_name
# 单参数非线性函数
lfuns = ['fixed_sin','random_sin','ranged_sin']
...
for lr in [*np.linspace(0.01, 0.05, 5),*np.linspace(0.001, 0.005, 5),*np.linspace(0.0001, 0.0005, 5)]:
    for lfun in lfuns:
        if lfun == 'ranged_sin':
            skan = importlib.import_module("skan_exp_version")
            base_function = getattr(skan, lfun)
            SKANNetwork = getattr(skan, 'SKANNetwork_ranged')
            net = SKANNetwork([784, 100, 10], base_function=base_function).to(device)
        elif lfun == 'random_sin':
            skan = importlib.import_module("skan_exp_version")
            base_function = getattr(skan, lfun)
            SKANNetwork = getattr(skan, 'SKANNetwork_random')
            net = SKANNetwork([784, 100, 10], base_function=base_function).to(device)

        else:
            skan = importlib.import_module("skan_exp_version")
            base_function = getattr(skan, lfun)
            SKANNetwork = getattr(skan, 'SKANNetwork')
            net = SKANNetwork([784, 100, 10], base_function=base_function).to(device)
